...
class CouponLogController extends Controller
{
    ...
    public function index(CouponLog $coupon_log_model,UserService $user_service){
        ...
        $list = $coupon_log_model->where('start_time','<',now())->where('end_time','>',now())->where('user_id',$user_info['id'])->orderBy('status','asc')->orderBy('id','desc')->paginate(request()->per_page??30);

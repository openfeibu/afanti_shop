...
class GoodsController extends Controller
{
    ...
    public function goods_info($id){
        ...
        if($goods_info['status']){
            $goods_info['data']['store_info'] = $this->store_service->getStoreInfoAndRate($goods_info['data']['store_id'],'id,store_name,store_company_name,area_info,store_address,after_sale_service')['data'];
            $goods_info['data']['sale_list'] = $this->goods_service->getSaleSortGoods(['class_id'=>$goods_info['data']['class_id']])['data']; // 销售排名
            $goods_info['data']['coupons'] = $this->coupon_service->getCouponByStoreId($goods_info['data']['store_id'])['data']; // 优惠券
            ...
            $goods_info['data']['collective_list'] = $this->collective_service->getCollectiveLogByGoodsId($id)['data']; // 正在团的

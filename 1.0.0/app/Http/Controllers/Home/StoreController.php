    ...
    public function get_store_info(StoreService $store_service,GoodsService $goods_service,$id){
        ...
        if($store_info['status']){
            ...
            if(!empty($store_info['data']['monitoring_channel']))
            {
                ...
                $config_info = $config_model->getFormatConfig('monitoring');
                $equipment_url = 'ezopen://'.$config_info['code'].'@open.ys7.com/'.$config_info['sn'].'/'.$store_info['data']['monitoring_channel'].'.live';
                $store_info['data']['equipment_url'] = $equipment_url;
                $store_info['data']['access_token'] = $access_token;
            }
